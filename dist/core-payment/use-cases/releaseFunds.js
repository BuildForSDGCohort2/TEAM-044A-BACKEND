...
exports.default = makeReleaseFunds;

/* eslint-disable no-underscore-dangle */

/* eslint-disable no-undef */
function makeReleaseFunds({
  escrowDb,
  moment,
  setCronJob,
  transactionDb
}) {
  return async function releaseFunds({
    referenceId
  }) {
    /**
     ...
     *
     */
    const receiver = await escrowDb.findByRef({
      ref: referenceId
    });
    ...
    if (inspectionPeriod === moment(Date.now()).valueOf()) {
      ...
      const elapsedHours = moment(Date.now()).add(24, 'hours'); // send money to seller

      if (elapsedHours) {
        setCronJob(await escrowDb.handleMoneyTransfer({
          referenceId,
          receiverId,
          amount
        }));
