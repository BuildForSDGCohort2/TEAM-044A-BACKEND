...
const makeInProgress = ({
  ...
  return async function inProgress({
    user,
    ref
  }) {
    const {
    ...
    const foundTransaction = await transactionDb.findAll(); // map through the returned documentss

    const userArr = foundUser.map(el => el.email).filter(person => person !== null);
    const transactionArr = foundTransaction.map(item => item.email).filter(item => item !== null);
